...
export function getGroqApiKey(): string {
  ...
  if (!apiKey) {
    throw new Error("[Groq Service] GROQ_API_KEY (or GEMINI_API_KEY) is not configured in environment.");
  }
  return apiKey;
}
 ...
 */
async function callGroqChat(messages: { role: string; content: string }[], temperature = 0.2): Promise<string> {
  const apiKey = getGroqApiKey();
  const modelsToTry = [activeWorkingModel, ...CANDIDATE_MODELS.filter((m) => m !== activeWorkingModel)];
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (
          ...
          console.warn(`[Groq Service] Model ${model} returned ${response.status} (${errorText.slice(0, 120)}...). Trying next candidate model...`);
          lastError = new Error(`Groq API error ${response.status}: ${errorText}`);
          continue;
        }

        console.error(`[Groq Service] Groq API returned ${response.status}: ${errorText}`);
      ...
      if (!content) {
        throw new Error("[Groq Service] Empty completion content returned by Groq.");
      }
  ...
  console.warn("[Groq Service] All candidate models busy. Waiting 2.5s before final retry...");
  ...
  throw lastError || new Error("[Groq Service] All candidate Groq models failed.");
}

/**
 * Tests Groq API connectivity and latency.
 */
export async function testGroqConnection(): Promise<{ success: boolean; model: string; message: string; latencyMs?: number }> {
  const startTime = Date.now();
  try {
    const text = await callGroqChat([
      { role: "system", content: "You are a health check system. Return valid JSON: {\"status\": \"ok\", \"service\": \"Groq\"}" },
...
Respond ONLY in JSON format:
{
  ...
  const systemInstruction = "You are Priora's Executive Email Ghostwriter. Draft clear, polite, and effective responses matching the specified tone. Return valid JSON only with 'draftText' and 'intentStrategy' keys.";
...
function normalizeAnalysis(data: Record<string, unknown>, input: ThreadAnalysisInput): GeminiEmailAnalysis {
  ...
  const reason = typeof data.reason === "string" ? data.reason : "Evaluated by Priora AI Intelligence.";
