...
export function ThreadFeedList({
  ...
  return (
    <div className="flex flex-col h-full border-r border-slate-800/80 bg-slate-950/60 min-w-0">
      {/* Header & Filter Tabs */}
      <div className="p-3 space-y-3 border-b border-slate-800/80">
        ...
        <div className="relative">
          ...
          <input
            ...
            className="w-full rounded-md border border-slate-800 bg-slate-900/90 pl-8 pr-3 py-1.5 text-xs text-slate-100 placeholder:text-slate-500 focus-ring"
          />
        </div>
      </div>
          ...
          filteredThreads.map((thread) => {
            ...
            return (
              <button
                ...
                className={cn(
                  "w-full text-left p-3.5 transition-colors focus-ring relative group cursor-pointer",
                  isSelected
                    ? "bg-slate-800/90 border-l-2 border-l-indigo-500 shadow-2xs"
              ...
              >
                ...
                <p className="text-xs font-medium text-slate-200 truncate mt-1 group-hover:text-indigo-300 transition-colors">
                  {thread.subject}
                </p>

                <p className="text-2xs text-slate-400 line-clamp-2 mt-1 leading-relaxed">
                  {thread.snippet}
                </p>

                <div className="flex items-center space-x-2 mt-2 pt-1">
                  <span className="inline-flex items-center text-2xs font-medium text-sky-400 bg-sky-500/10 px-1.5 py-0.5 rounded">
